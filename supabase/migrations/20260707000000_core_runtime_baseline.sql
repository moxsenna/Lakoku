...
' order by required.table_name,required.column_name))
    ...
    if (select md5(string_agg(c.relname||'|'||pg_catalog.pg_get_userbyid(c.relowner)||'|'||c.relrowsecurity::text||'|'||coalesce(array_to_string(c.relacl,','),''),E'\n' order by c.relname)) from pg_catalog.pg_class c join pg_catalog.pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname=any(v_owned_tables))<>'cf4d8e1722ca3c5afdb956d4977bf31d'
    ...
    if md5(pg_catalog.pg_get_functiondef('public.acquire_generation_lease(text,integer,text,integer,text)'::regprocedure))<>'a510dcaf674b178782dfe971aea57be2'
      ...
      or md5(pg_catalog.pg_get_functiondef('public.story_is_public(text)'::regprocedure))<>'73310f024aff9df0dcfd3995648954ad'
      or md5(pg_catalog.pg_get_functiondef('public.story_is_owned_by_auth(text)'::regprocedure))<>'25bf18127829a221b1db9d9dc67dcd85'
      or (select count(*) from pg_catalog.pg_proc p where p.oid=any(array(select pg_catalog.to_regprocedure('public.'||x) from unnest(v_owned_functions[1:3]) x)) and pg_catalog.pg_get_userbyid(p.proowner)='postgres' and p.prosecdef and p.provolatile='v' and p.proconfig=array['search_path=public'] and p.proacl=array['postgres=X/postgres','service_role=X/postgres']::aclitem[])<>3
      ...
      or has_function_privilege('anon','public.acquire_generation_lease(text,integer,text,integer,text)','EXECUTE') or has_function_privilege('authenticated','public.acquire_generation_lease(text,integer,text,integer,text)','EXECUTE') or not has_function_privilege('service_role','public.acquire_generation_lease(text,integer,text,integer,text)','EXECUTE')
      ...
      or has_function_privilege('anon','public.publish_chapter(text,integer,text,jsonb,text,jsonb,jsonb,uuid,text)','EXECUTE') or has_function_privilege('authenticated','public.publish_chapter(text,integer,text,jsonb,text,jsonb,jsonb,uuid,text)','EXECUTE') or not has_function_privilege('service_role','public.publish_chapter(text,integer,text,jsonb,text,jsonb,jsonb,uuid,text)','EXECUTE')
