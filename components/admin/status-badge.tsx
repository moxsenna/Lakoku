...
const statusMap: Record<string, { label: string; className: string }> = {
  ...
  // Provider-call outcomes — NOT end-to-end publish success.
  SUCCEEDED: { label: 'Provider OK', className: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' },
  BAB_TERBIT: { label: 'Bab terbit', className: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' },
  MASIH_DIPROSES: { label: 'Masih diproses', className: 'bg-sky-500/10 text-sky-600 dark:text-sky-400' },
  DITAHAN_PEMERIKSAAN: { label: 'Ditahan pemeriksaan', className: 'bg-amber-500/10 text-amber-600 dark:text-amber-400' },
  PROSES_GAGAL: { label: 'Proses gagal', className: 'bg-red-500/10 text-red-600 dark:text-red-400' },
  BELUM_TERBIT: { label: 'Belum terbit', className: 'bg-muted text-muted-foreground' },
  TIDAK_DIKETAHUI: { label: 'Tidak diketahui', className: 'bg-muted text-muted-foreground' },
