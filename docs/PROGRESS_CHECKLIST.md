...
- [x] **Exit Criteria M4** — generasi AI satu bab lolos Layer A + repair; string consumer-safe.
  - **TERBUKTI**: `scripts/m4-generation.ts` — 27/27 PASS. Happy path Bab 6 (0 repair, hard 800–1000 kata), reveal gate Bab 12, karakter baru terencana Bab 33, repair menyembuhkan MAJOR (1 attempt), provider bandel → FAILED_REVIEW_REQUIRED (2 attempt, draft null), schema tolak plan/draft invalid, consumer-safe lolos & tangkap kebocoran, template 50-bab konsisten. `tsc --noEmit` hijau; `eslint` 0 error/0 warning.
...
- [x] **T5.3 Thread lifecycle** — NTM G4-STATUS, G4-BUDGET, G4-STALE, G4-BLOCK48 — status OPEN→…→RESOLVED|ABANDONED_APPROVED; maks 7 thread; no new thread ≥ Bab 41; stale 6 bab → callback ≤ 3 bab; Bab 48 wajib menghasilkan post-chapter state dengan mystery utama RESOLVED (pre-chapter snapshot tidak memblok closure bab yang sama); Bab 49+ unresolved hard-blocked.
  - **TERBUKTI**: `lib/narrative/threads.ts` — status machine `OPEN→DEVELOPING→PAYOFF_DUE→RESOLVED|ABANDONED_APPROVED` (`transitionThread` menolak transisi ilegal; `ABANDONED_APPROVED` hanya via checkpoint). Budget maks 7 aktif (`THREAD_BUDGET_EXCEEDED`/`canOpenNewThread`), no-new ≥ Bab 41 (`THREAD_NEW_FORBIDDEN`), stale 6 bab → callback ≤ 3 (`refreshStaleness`/`THREAD_STALE_UNADDRESSED`), Act 6+ wajib majukan PAYOFF_DUE. Otoritas Bab 48: `resolveDebtClosures()` (`MAIN_MYSTERY_UNRESOLVED`, resolve-by 48) menolak publication Bab 48 bila delta tidak menutup mystery utama; Bab 49+ unresolved tetap hard-blocked — bukti: `m10-a1d` negative regression (sync + worker, gagal sebelum publication).
