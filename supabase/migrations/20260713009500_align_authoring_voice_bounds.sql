...
begin
  ...
  if exists (
    ...
    where not (item ?& array['character_id','register','speech_habits','forbidden_words','sample_lines'])
      ...
      or jsonb_typeof(item->'character_id') <> 'string' or char_length(item->>'character_id') not between 1 and 256
      or jsonb_typeof(item->'register') <> 'string' or char_length(item->>'register') not between 3 and 140
      or jsonb_typeof(item->'speech_habits') <> 'array' or jsonb_array_length(item->'speech_habits') > 6
      or jsonb_typeof(item->'forbidden_words') <> 'array' or jsonb_array_length(item->'forbidden_words') > 10
      or jsonb_typeof(item->'sample_lines') <> 'array' or jsonb_array_length(item->'sample_lines') not between 1 and 4
      or exists (select 1 from jsonb_array_elements(item->'speech_habits') v where jsonb_typeof(v) <> 'string' or char_length(v #>> '{}') not between 2 and 120)
      ...
      or exists (select 1 from jsonb_array_elements(item->'sample_lines') v where jsonb_typeof(v) <> 'string' or char_length(v #>> '{}') not between 3 and 200)
      or (item ? 'story_id' and (jsonb_typeof(item->'story_id') <> 'string' or btrim(item->>'story_id') <> v_story_id))
  ) or exists (
