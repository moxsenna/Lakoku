...
create table private.generation_incident_captures (
  ...
  constraint generation_incident_captures_version_check check (
  ...
  constraint generation_incident_captures_chapter_check check (
    chapter_number between 1 and 50
  ),
  constraint generation_incident_captures_choice_index_check check (
    choice_index between 0 and 31
  ),
  constraint generation_incident_captures_stage_check check (
    stage = 'FINAL_BRANCH_SCHEMA'
  ),
  constraint generation_incident_captures_code_check check (
    code = 'CHOICE_NOT_ACTIONABLE'
  ),
  constraint generation_incident_captures_ciphertext_check check (
    pg_catalog.length(ciphertext) between 1 and 4096
    and ciphertext ~ '^[A-Za-z0-9_+/-]+={0,2}$'
  ),
  constraint generation_incident_captures_nonce_check check (
    pg_catalog.length(nonce) between 16 and 32
    and nonce ~ '^[A-Za-z0-9_+/-]+={0,2}$'
  ),
  constraint generation_incident_captures_auth_tag_check check (
    pg_catalog.length(auth_tag) between 16 and 64
    and auth_tag ~ '^[A-Za-z0-9_+/-]+={0,2}$'
  ),
  constraint generation_incident_captures_ttl_check check (
    expires_at > created_at
    and expires_at <= created_at + interval '60 minutes'
  ),
  constraint generation_incident_captures_consumed_check check (
...
on private.generation_incident_captures(consumed_at, capture_id)
...
revoke all on table private.generation_incident_captures
  from public, anon, authenticated, service_role;
revoke all on table private.generation_incident_access_audit
  from public, anon, authenticated, service_role;
...
begin
  if p_capture_id is null
    ...
    or p_expires_at <= v_created_at
    or p_expires_at > v_created_at + interval '60 minutes' then
  ...
  insert into private.generation_incident_captures (
    ...
    p_chapter_number, p_choice_index, p_stage, p_code, p_ciphertext, p_nonce,
    p_auth_tag, v_created_at, p_expires_at
  )
...
create function public.consume_generation_incident_v1(
  p_capture_id uuid,
  p_correlation_id uuid
)
returns table (
...
begin
  ...
  if p_capture_id is null or p_correlation_id is null then
    raise exception using errcode = 'P0001', message = 'INVALID_INCIDENT_LOOKUP';
  end if;
  ...
  where captures.capture_id = p_capture_id
    ...
    and captures.expires_at > v_consumed_at
  ...
  insert into private.generation_incident_access_audit (
    ...
    v_row.capture_id, v_row.correlation_id, v_actor, 'CONSUMED', v_consumed_at
  );
  ...
  return query select
    ...
    v_row.incident_key,
    v_row.label_fingerprint,
    v_row.version,
    v_row.story_id,
    v_row.chapter_number,
    v_row.choice_index,
    v_row.stage,
    v_row.code,
    v_row.expires_at,
...
create function public.cleanup_generation_incidents_v1(
...
begin
  ...
  with selected as (
    ...
    from private.generation_incident_captures as captures
    where captures.consumed_at is not null
       or captures.expires_at <= v_now
    ...
    select
      ...
      null,
      case when captures.consumed_at is null then 'EXPIRED' else 'PURGED' end,
      v_now
    from private.generation_incident_captures as captures
    join selected using (capture_id)
    where captures.consumed_at is null
       or not exists (
         select 1
         from private.generation_incident_access_audit as audit
         where audit.capture_id = captures.capture_id
           and audit.action = 'PURGED'
       )
    returning capture_id
  ), deleted as (
    delete from private.generation_incident_captures as captures
    using selected
    where captures.capture_id = selected.capture_id
    returning captures.capture_id
  )
...
revoke all on function public.capture_generation_incident_v1(
  uuid,uuid,text,text,integer,text,integer,integer,text,text,text,text,text,timestamptz
) from public, anon, authenticated, service_role;
grant execute on function public.capture_generation_incident_v1(
  uuid,uuid,text,text,integer,text,integer,integer,text,text,text,text,text,timestamptz
) to service_role;

revoke all on function public.consume_generation_incident_v1(uuid,uuid)
  from public, anon, authenticated, service_role;
grant execute on function public.consume_generation_incident_v1(uuid,uuid)
  to authenticated;

revoke all on function public.cleanup_generation_incidents_v1(integer)
  from public, anon, authenticated, service_role;
grant execute on function public.cleanup_generation_incidents_v1(integer)
  to service_role;
