...
| G1-REACH | Ending reachability check tiap checkpoint | `ending_rules` | Reconciliation step | soak 3 jalur | "semua ending reachable" (NCS §8.3) | soak | IN_PROGRESS (`checkEndingReachability()` dan soak 3 jalur×50 bab hijau. Post-publication lifecycle M10-C sudah mem-persist `ACT_ENDING_REACHABILITY` tiap act boundary, TAPI bukti itu **NOT RATIFIED** (reviewer ledger Entry 6) dan bukan PASS: `EndingCandidateSchema` hanya punya `key/name/condition/requiredClosure`, tidak bisa menyatakan secret ending maupun blocking terstruktur, sehingga NCS §1.4 tak terbuktikan pada model kontrak sekarang — payload merekam `ncs14Proven: false` + `secretPath UNPROVEN`. `DONE` menuntut perubahan model kontrak ending, bukan sekadar step runtime.) |
