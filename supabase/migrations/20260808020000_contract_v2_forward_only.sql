-- C-R3-R1: Forward-only migration for contract v2 support
-- 
-- Created: 2026-08-08
-- Purpose: Add structured ending model (kind, isSecret, blockingConditions)
--          without modifying historical applied migration 20260728010000
--
-- Authorizes:
--   - styleProfile = 'lakoku_mobile_drama_v2'
--   - ending candidates with kind ('main'|'secret'), isSecret, blockingConditions[]
--   - requires at least 2 main endings when using v2 schema
--
-- N.B.: This migration only affects new story bootstraps via bootstrap_personalized_story_v2
--       Existing v1 stories remain compatible via explicit versioned schemas.

-- ──────────────────────────────────────────────────────────────────────────────
-- 1. Create bootstrap function for contract v2
-- ──────────────────────────────────────────────────────────────────────────────
-- Versioned separately from v1 to allow forward-only evolution.
-- Does NOT modify bootstrap_personalized_story_v1.

create or replace function public.bootstrap_personalized_story_v2(
  p_story_id text,
...
begin
  ...
  if pg_catalog.jsonb_typeof(p_onboarding_json) is distinct from 'object'
    ...
    or exists (select 1 from pg_catalog.jsonb_object_keys(p_story_contract_json) k where k not in (
      ...
      'revealRunway','closureRunway','plotDebts','endingCandidates'
    ))
    ...
    or p_story_contract_json->>'styleProfile' is distinct from 'lakoku_mobile_drama_v2'
    ...
    or pg_catalog.jsonb_typeof(p_story_contract_json->'plotDebts') <> 'array'
    or pg_catalog.jsonb_array_length(p_story_contract_json->'plotDebts') not between 1 and 20
    or pg_catalog.jsonb_typeof(p_story_contract_json->'endingCandidates') <> 'array'
    or pg_catalog.jsonb_array_length(p_story_contract_json->'endingCandidates') not between 2 and 8
  then
    raise exception using errcode = '22023', message = 'INVALID_CONTRACT';
  end if;

  -- Exact nested contract keys and scalar bounds for v2 (styleProfile = v2 required).
  if exists (
    ...
    or (
        select count(*) from pg_catalog.jsonb_array_elements(p_ending_candidates_json) item
        where (item->>'kind') = 'main'
    ) < 2
    -- Enforce at least 1 secret ending path per NCS §1.4
    or (
        select count(*) from pg_catalog.jsonb_array_elements(p_ending_candidates_json) item
        where (item->>'kind') = 'secret'
    ) < 1
    or exists (
  ...
  insert into public.story_generation_contracts(
    ...
    'lakoku_mobile_drama_v2',v_payload_hash,v_story.story_contract_version
  );
...
comment on function public.bootstrap_personalized_story_v2(text,uuid,text,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb) is
  'Bootstrap personalized story contract v2 with structured ending model (kind/isSecret/blockingConditions). Forward-only, does not modify historical migration.';

-- Grant execute only to service_role; revoke all others.
revoke all on function public.bootstrap_personalized_story_v2(text,uuid,text,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb) from public, anon, authenticated;
grant execute on function public.bootstrap_personalized_story_v2(text,uuid,text,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb) to service_role;
