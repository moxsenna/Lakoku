...
import { MAX_REPAIR_ATTEMPTS } from '../../ai-gateway/generate'
...
export type InjectionProbe = () => void

/** Initial prose write plus current production Layer A repair ceiling. */
export const PERSISTENT_SHORT_PRODUCTION_WRITE_CEILING = 1 + MAX_REPAIR_ATTEMPTS
...
export function providerThrowingBeforeFirstByte(onInjected: InjectionProbe = () => {}): GenerationProvider {
...
export function providerThrowingAfterPartial(onInjected: InjectionProbe = () => {}): GenerationProvider {
  const base = createDeterministicProvider()
  return {
    ...base,
...
export function providerRetryable429(onInjected: InjectionProbe = () => {}): GenerationProvider {
  const base = createDeterministicProvider()
  return {
    ...base,
    name: 'm10e-fault-429',
...
export function providerNonRetryable(onInjected: InjectionProbe = () => {}): GenerationProvider {
  const base = createDeterministicProvider()
  return {
    ...base,
...
export function providerMalformedProse(onInjected: InjectionProbe = () => {}): GenerationProvider {
  const base = createDeterministicProvider()
  return {
    ...base,
 ...
 */
export function providerFirstWriteDefective(
  defects: DraftDefect[],
  onInjected: InjectionProbe = () => {},
): GenerationProvider {
  const base = createDeterministicProvider()
  return {
    ...base,
 ...
 */
export function providerPersistentlyShort(onInjected: InjectionProbe = () => {}): GenerationProvider {
  const base = createDeterministicProvider()
  return {
    ...base,
