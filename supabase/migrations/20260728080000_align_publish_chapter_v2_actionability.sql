  ...
  if p_chapter_number = 50 then
    ...
    loop
      ...
      if pg_catalog.lower(v_choice->>'label') ~
        ...
        or pg_catalog.lower(v_choice->>'label') !~
          '^(?:(tetap|terus|segera|langsung|perlahan|diam-diam|cepat|coba|kembali|maju|mundur|pura-pura|tiba-tiba)[[:space:]]+)?(me[a-z]{2,}|ber[a-z]+|di[a-z]+|ter[a-z]+|hadapi|selidiki|buka|tutup|ambil|amankan|tinggalkan|ikuti|hadang|tanya|tolong|selamatkan|lawan|kejar|periksa|baca|sembunyikan|ungkapkan|masuk|keluar|lari|panggil|cari|pilih|tolak|terima|kirim|hancurkan|jaga|dekati|hindari|putuskan|diam|dengar|lihat|coba|pegang|tarik|dorong|lempar|tangkap|lepas|gunakan|pakai|naik|turun|berhenti|mulai|lanjutkan|hentikan|kunci|kumpulkan|sebarkan|bagikan|ceritakan|jelaskan|tanyakan|katakan|ucapkan|beritahu|balas|tembak|serang|lompat|teriak|bisik|usir|rayu|gali|gantung|bakar|potong|tancap|tuang|isi|kosongkan|bersihkan|perbaiki|tusuk|tikam|pukul|tampar|seret|jatuhkan|bangun|tidur|makan|minum|bawa|datang|pergi|pulang|kembali|singgah|hadir|muncul|tampil|lenyap|hilang|bersembunyi|membuntuti|berjalan|berlari|bertanya|berlindung|berjuang|berteriak|berbisik|berdoa|open|close|take|leave|follow|stop|ask|help|save|fight|chase|inspect|read|hide|reveal|enter|run|call|find|choose|refuse|accept|send|destroy|guard|approach|avoid)([^[:alnum:]_]|$)' then
