-- Align publish_chapter_v2 choice actionability validator with full TypeScript lexicon
-- (269 root imperatives, peN-/per- prefixes, -kan/-lah/-i verbal suffixes, and internal term guards).
    ...
    loop
      ...
      if pg_catalog.lower(v_choice->>'label') ~
        ...
        or pg_catalog.lower(v_choice->>'label') ~
          '(^|[^[:alnum:]_])(route|risk[[:space:]]+state|flag[[:space:]]+state)([^[:alnum:]_]|$)|^(set[[:space:]]+flag|naikkan[[:space:]]+risk[[:space:]]+state)|(naikkan|turunkan|set|reset)[[:space:]]+(risk|state|flag|route)'
        or pg_catalog.lower(v_choice->>'label') ~
          '^(?:(tetap|terus|segera|langsung|perlahan|diam-diam|cepat|coba|kembali|maju|mundur|pura-pura|tiba-tiba)[[:space:]]+)?(merasa|berharap|menginginkan|mendambakan|membayangkan|merenungkan|memikirkan|menghayalkan|mengkhawatirkan|menyesali|menerka|menduga|mengira|menyangka|mengkhayalkan|merindukan|mengharapkan|pikirkan|renungkan|bayangkan|harapkan|dambakan|inginkan|khayalkan|khawatirkan|impikan|rindukan|pekan|pakan|kopi|kaki|nasi|sapi|padi|sepi|mini|taxi|ialah|adalah)([^[:alnum:]_]|$)'
        or pg_catalog.lower(v_choice->>'label') !~
          '^(?:(tetap|terus|segera|langsung|perlahan|diam-diam|cepat|coba|kembali|maju|mundur|pura-pura|tiba-tiba)[[:space:]]+)?(me[a-z]{2,}|ber[a-z]+|di[a-z]+|ter[a-z]+|pe[a-z]{2,}|[a-z]{3,}kan|[a-z]{3,}lah|[a-z]{3,}i|abaikan|accept|ajak|ajar|amankan|amati|ambil|ancam|angkat|antar|approach|ask|avoid|baca|bagikan|bakar|balas|balik|balut|bangun|banting|bantu|batalkan|bawa|berbisik|berdoa|berhenti|beritahu|berjalan|berjuang|berlari|berlindung|bersembunyi|bersihkan|bertanya|berteriak|bilas|bimbing|bisik|bongkar|buang|bujuk|buka|buru|call|cari|catat|cegat|cek|cengkeram|ceritakan|cermati|chase|choose|close|coba|colek|colong|copot|cubit|cuci|curi|datang|datangi|dekati|dengar|desak|destroy|diam|dorong|duduk|dukung|elus|endus|enter|fight|find|follow|gali|gantung|gapai|gebuk|gedor|geledah|genggam|geser|gigit|giring|goncang|gores|gosok|goyang|guard|gunakan|hadang|hadapi|hadir|hajar|halau|hancurkan|hantam|help|hembus|hentak|hentikan|hide|hilang|hindari|hisap|hitung|ikat|ikuti|injak|inspect|intip|isi|jaga|jatuhkan|jawab|jebak|jelaskan|jemput|jerat|jongkok|kabur|kait|kaitkan|kasih|katakan|kejar|keluar|kembali|kenali|kerahkan|ketuk|kira|kirim|konfrontasi|kosongkan|kubur|kumpulkan|kunci|kupas|kurung|lacak|lanjutkan|lap|lari|latih|lawan|leave|lempar|lenyap|lepas|lewat|lihat|lindungi|lirik|lompat|makan|masuk|masukkan|membuntuti|minta|minum|mulai|muncul|naik|nyalakan|nyatakan|open|pakai|paksa|pancing|pandang|panggil|panjat|pasang|pegang|peluk|penuhi|perbaiki|pergi|periksa|petik|pijak|pikir|pikul|pilih|pinjam|potong|pujuk|pukul|pulang|putar|putuskan|racik|raih|rampas|rampok|ransak|rayu|read|rebah|rebut|refuse|rekam|remas|renggut|reveal|robek|run|sabotase|sadap|salin|sambar|sambut|save|sebarkan|sebut|selamatkan|selidiki|sembunyi|sembunyikan|send|sentuh|serang|seret|sergap|siapkan|sikat|simak|simpan|singgah|sisir|sobek|sorot|stop|suntik|susun|susup|tagih|tahan|take|tampar|tampil|tampilkan|tancap|tanggung|tangkap|tangkis|tanya|tanyakan|tarik|tatap|tawan|tawar|tegur|tekan|tembak|tempel|temui|tendang|tengok|tepis|tepuk|teriak|terima|tidur|tikam|timbang|timbun|tinggalkan|titip|tiup|tolak|tolong|tuang|tukar|tunggu|tunjuk|tuntun|turun|tusuk|tutup|ubah|ucapkan|ugut|uji|ukir|ukur|ulang|undang|ungkapkan|usir|waspadai)([^[:alnum:]_]|$)' then
