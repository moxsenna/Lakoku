...
export function buildWriterLengthRepairPrompt(args: Readonly<{
  ...
  const neededWords = Math.max(160, 920 - args.wordCount)
  const neededParagraphs = Math.max(15, Math.ceil(neededWords / 13))
  const direction = args.wordCount < 800
    ? `PERINGATAN KERAS: Draf pertama (${args.wordCount} kata) KURANG PANJANG dan GAGAL lolos (batas minimal mutlak sistem adalah 800 kata).\n` +
      ...
      `- Targetkan total struktur naskah memiliki 65–80 paragraf pendek agar panjang kata stabil di 850–950 kata.`
