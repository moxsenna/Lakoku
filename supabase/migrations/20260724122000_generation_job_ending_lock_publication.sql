-- Publish a fenced generation job and optional personalized chapter-45 ending lock atomically.

create function public.publish_generation_job_chapter_v3(
  p_job_id uuid,
  p_worker_id text,
  p_claim_token uuid,
  p_lease_id uuid,
...
begin
  ...
  -- Idempotent success precedes live ownership, lease, target, and ending-lock validation.
  ...
  if v_has_ending_lock then
    ...
    if v_job.generation_kind is distinct from 'personalized'
      or v_job.chapter_number is distinct from 45 then
