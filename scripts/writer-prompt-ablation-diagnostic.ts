...
const ARTIFACT_DIR = path.resolve(
  process.cwd(),
  '.zcode/artifacts/m10-f-writer-prompt-ablation/2026-09-03-writer-prompt-ablation-v1',
)
...
async function main(): Promise<void> {
  ...
  const authority = {
    ...
    diagnosticChildFlag: process.env.LAKOKU_WRITER_PROMPT_ABLATION_CHILD,
    credentialAvailable: Boolean(process.env.OPENROUTER_API_KEY?.trim()),
  }
  const preflight = await preflightWriterPromptAblation(authority)

  console.log('WRITER_PROMPT_ABLATION_V1_PREFLIGHT', JSON.stringify({
    ok: preflight.ok,
  ...
  const route = createWriterPromptAblationRoute()
  const provider = createGatewayProvider(
    { model: WRITER_PROMPT_ABLATION_V1_CONFIG.modelId },
    { targetWordsMin: 850, targetWordsMax: 950, targetScenes: 3 },
    route,
  )
  const report = await executeWriterPromptAblation({ ...authority, provider })
  assertWriterPromptAblationSerialization(report)

  mkdirSync(ARTIFACT_DIR, { recursive: true })
  const artifactPath = path.join(ARTIFACT_DIR, 'result.json')
  writeFileSync(artifactPath, `${JSON.stringify(report, null, 2)}\n`, 'utf8')
  console.log('WRITER_PROMPT_ABLATION_V1_RESULT', JSON.stringify({
...
void main().catch((error: unknown) => {
  console.error(
    'WRITER_PROMPT_ABLATION_V1_ABORT',
