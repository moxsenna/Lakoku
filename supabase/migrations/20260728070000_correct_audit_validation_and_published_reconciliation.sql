-- Correct audit constraint rollout and strengthen durable PUBLISHED reconciliation.
...
-- Validation intentionally fails and rolls this migration back if existing rows violate contract.
alter table public.chapter_generation_checkpoints
  validate constraint chapter_generation_checkpoints_audit_signals_check;
...
begin
  ...
  if v_is_publish then
    -- Load persisted checkpoint payload before validating durable publication proof.
    ...
    if v_job.status <> 'SUCCEEDED'
      ...
      or not exists (
        ...
        where c.story_id = v_job.story_id
          ...
          and v_job.publication_payload_hash is not null
          and v_job.closure_payload_hash is not null
          and c.title is not distinct from v_checkpoint.title
          and c.paragraphs is not distinct from v_checkpoint.paragraphs_json
      ) then
